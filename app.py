...
class ComplianceEngine:
    ...
    def calculate(self, sys, g_in, g_out, c_mob, c_mot, hours, rev, mgt_fee_rate=0.0):
        ...
        # 2. Staff / Office Logic
        spv, adm, cpm = (3, 1, 1) if rev >= 500000000 else (1, 1, 0) if rev >= 150000000 else (0, 1, 0)
        
        # 3. IT/Technician Logic (New Rule: Manual = 0, Small Location = 0)
        it_tech = 1 if (sys != 'Manual' and rev >= 150000000) else 0 

        # Allowance Logic (Dynamic: highest position gets 30%)
        adm_all, spv_all, cpm_all, it_all = 0.10, 0.20, 0.30, 0.10
        if cpm > 0: pass
        elif spv > 0: spv_all = 0.30
        elif it_tech > 0 and it_tech > adm: it_all = 0.30 # Theoretical case
        elif adm > 0: adm_all = 0.30

        total_actual_cost = ( (cashier + ctrl + att) * self.get_individual_cost(0) ) + \
        ...
        return {
            "mpp": cashier + ctrl + att + spv + adm + cpm + it_tech, 
...
st.markdown("Strict Industry Compliance: Automated IT Deployment & Actual P&L Logic")
st.divider()
...
with st.sidebar:
    ...
    c1, c2 = st.columns(2)
    with c1: c_mob = st.number_input("Car Capacity", value=500, step=50); g_in = st.number_input("Gate IN", value=3)
    with c2: c_mot = st.number_input("Motorcycle Capacity", value=200, step=50); g_out = st.number_input("Gate OUT", value=3)
...
def render_scenario(label, suffix, h_class, cost_class, b_class, def_rev, def_sys):
    st.markdown(f"<div class='{h_class}'><h3 class='header-title'>{label}</h3></div>", unsafe_allow_html=True)
    sys = st.selectbox(f"System {label[-1]}", ['Manual', 'Semi-Auto', 'Full Manless'], index=def_sys, key=f"s_{suffix}")
    ...
    html = f"""<div class='result-card'><div class='metric-row'><div class='metric-card {b_class}'><div class='metric-label'>Total MPP</div><div class='metric-value'>{res['mpp']} Pax</div></div><div class='metric-card {b_class}'><div class='metric-label'>Cost Ratio</div><div class='metric-value'>{res['ratio']:.2f}%</div></div></div><div class='breakdown-bar'><div class='bd-item'><span class='bd-label'>Ops Shift</span><span class='bd-val'>{res['shift_mpp']} Pax</span></div><div class='bd-item bd-border'><span class='bd-label'>Per Group</span><span class='bd-val'>{get_shift_distribution(res['shift_mpp'])}</span></div><div class='bd-item'><span class='bd-label'>Office</span><span class='bd-val'>{res['office_mpp']} Pax</span></div></div><div style='display:flex; gap:10px; margin-bottom:15px;'><div class='comp-box' style='flex:1;'><div class='comp-title'>Non-Staff</div><div class='comp-row'><span>Attendant</span><span class='comp-val'>{res['att']}</span></div><div class='comp-row'><span>Cashier</span><span class='comp-val'>{res['cashier']}</span></div><div class='comp-row'><span>Control</span><span class='comp-val'>{res['ctrl']}</span></div></div><div class='comp-box' style='flex:1;'><div class='comp-title'>Staff</div><div class='comp-row'><span>Admin</span><span class='comp-val'>{res['adm']}</span></div><div class='comp-row'><span>SPV</span><span class='comp-val'>{res['spv']}</span></div><div class='comp-row'><span>CPM</span><span class='comp-val'>{res['cpm']}</span></div><div class='comp-row'><span>IT Tech</span><span class='comp-val'>{res['it_tech']}</span></div></div></div><div class='total-cost-box {cost_class}'><div class='total-label'>{cost_label}</div><p style='font-size: 1.6rem; font-weight: 900; color: {'#004a99' if 'a' in suffix else '#d97706'}; margin: 0;'>{format_idr(res['cost'])}</p></div></div>"""
    st.markdown(html, unsafe_allow_html=True)
    return res

with col_a: res_a = render_scenario("🅰️ Scenario A", "a", "header-card-a", "total-cost-a", "metric-card-a", 150000000, 0)
with col_b: res_b = render_scenario("🅱️ Scenario B", "b", "header-card-b", "total-cost-b", "metric-card-b", 500000000, 2)

# --- REPORT DOWNLOAD ---
st.divider()
def get_report(r_a, r_b):
    output = BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df = pd.DataFrame({
            "Metric": ["Project ID", "Property", "Total MPP", "Attendant", "Cashier", "Control Room", "Admin", "Supervisor", "CPM", "IT/Technician", "Cost Ratio (%)", "Total Cost"],
            "Scenario A": [p_name, p_type, r_a['mpp'], r_a['att'], r_a['cashier'], r_a['ctrl'], r_a['adm'], r_a['spv'], r_a['cpm'], r_a['it_tech'], round(r_a['ratio'],2), r_a['cost']],
            "Scenario B": [p_name, p_type, r_b['mpp'], r_b['att'], r_b['cashier'], r_b['ctrl'], r_b['adm'], r_b['spv'], r_b['cpm'], r_b['it_tech'], round(r_b['ratio'],2), r_b['cost']]
        })
        df.to_excel(writer, index=False)
    return output.getvalue()

st.download_button("📥 DOWNLOAD REPORT (EXCEL)", get_report(res_a, res_b), f"MPP_{p_name}.xlsx")
